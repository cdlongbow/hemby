...
/**
 * Utility class used to collect information about images in Emby API responses to easily pass around the app.
 */
data class EmbyImage(
	val item: UUID,
	val source: EmbyImageSource,
	val type: ImageType,
	val tag: String,
	val blurHash: String?,
...
fun EmbyImage.getUrl(
	...
	EmbyImageSource.USER -> api.imageApi.getUserImageUrl(
		userId = item,
		tag = tag,
	)
...
val BaseItemDto.channelPrimaryImage
	get() = channelPrimaryImageTag?.let { channelPrimaryImageTag ->
		channelId?.let { channelId ->
			EmbyImage(
				item = channelId,
				source = EmbyImageSource.CHANNEL,
				type = ImageType.PRIMARY,
...
val BaseItemDto.chapterImages
	get() = chapters?.mapIndexed { index, chapter ->
		EmbyImage(
			item = id,
			source = EmbyImageSource.CHAPTER,
